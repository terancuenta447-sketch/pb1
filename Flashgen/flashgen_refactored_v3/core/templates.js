 ...
 * Módulo: Templates
 * Categoría: core
 * Extraído de: Flashgen.js (líneas 6127-6936)
 * Generado automáticamente: 2025-11-28 11:46:46
 * 
 * Dependencias: API, DebugLogger, FewShotManager, ParserManager, Results, State, Storage, UI
 */
...
import { Learning } from '../ui/learning.js';
    ...
    **EJEMPLO INCORRECTO (EVITAR):**
    ...
    Respuesta: El tiempo es un concepto abstracto...`,
        ...
        init() {
            // ✅ FIX: Preservar plantillas guardadas y hacer merge con defaults
            if (!State.templates || Object.keys(State.templates).length === 0) {
                // Si no hay plantillas guardadas, usar defaults
                State.templates = { ...this.defaults };
            } else {
                // Si hay plantillas guardadas, hacer merge (guardadas tienen prioridad)
                State.templates = { ...this.defaults, ...State.templates };
            }
            
            // ✅ FIX: Validar que activeTemplate existe
            if (!State.activeTemplate || !State.templates[State.activeTemplate]) {
                State.activeTemplate = 'language';
                // Si 'language' no existe, usar la primera disponible
                if (!State.templates[State.activeTemplate]) {
                    const firstKey = Object.keys(State.templates)[0];
                    if (firstKey) {
            ...
            DebugLogger.log(`✅ Templates inicializado: ${Object.keys(State.templates).length} plantillas, activa: ${State.activeTemplate}`, 'success');
            this.updateUI();
        },
        ...
        updateUI() {
            ...
            if (select) {
                ...
                // ✅ FIX: Validar que hay plantillas
                if (!State.templates || Object.keys(State.templates).length === 0) {
                    DebugLogger.log('⚠️ No hay plantillas disponibles, inicializando defaults', 'warning');
                    State.templates = { ...this.defaults };
                    State.activeTemplate = 'language';
                }
                ...
                Object.keys(State.templates).forEach(key => {
                    ...
                    opt.value = key;
                    opt.textContent = State.templates[key]?.name || key;
                    select.appendChild(opt);
                });
                
                // ✅ FIX: Validar que activeTemplate existe antes de asignarlo
                if (State.activeTemplate && State.templates[State.activeTemplate]) {
            ...
            // ✅ FIX: Solo cargar si hay template activo válido
            if (State.activeTemplate && State.templates[State.activeTemplate]) {
                this.load(State.activeTemplate);
            } else {
                DebugLogger.log('⚠️ No hay template activo válido para cargar', 'warning');
            }
        },
        ...
        load(key) {
            ...
            if (!t) {
                ...
                UI.toast(`❌ Plantilla "${key}" no encontrada`, 'error');
                return;
            }
            
            State.activeTemplate = key;
            
            // ✅ FIX: Validar elementos DOM antes de usarlos
            const templateNameEl = document.getElementById('templateName');
            const systemPromptEl = document.getElementById('systemPrompt');
            const userPromptEl = document.getElementById('userPrompt');
            
            if (!templateNameEl || !systemPromptEl || !userPromptEl) {
                DebugLogger.log('⚠️ Elementos DOM de template no encontrados', 'warning');
                return;
            }
            ...
            // ✅ FIX: Ensure variables object exists
            if (!t.variables) {
                t.variables = { language: '', subject: '', difficulty: 'intermedio', bookTitle: '', chapter: '', chapterSummary: '', manualContext: '', targetLanguage: '' };
            }
            
            // ✅ FIX: Validar elementos DOM de variables antes de usarlos
            const varLanguageEl = document.getElementById('varLanguage');
            const varSubjectEl = document.getElementById('varSubject');
            const varDifficultyEl = document.getElementById('varDifficulty');
            const varBookTitleEl = document.getElementById('varBookTitle');
            const varChapterEl = document.getElementById('varChapter');
            const varChapterSummaryEl = document.getElementById('varChapterSummary');
            const varTargetLanguageEl = document.getElementById('varTargetLanguage');
            
            if (varLanguageEl) varLanguageEl.value = t.variables.language || '';
            if (varSubjectEl) varSubjectEl.value = t.variables.subject || '';
            if (varDifficultyEl) varDifficultyEl.value = t.variables.difficulty || '';
            if (varBookTitleEl) varBookTitleEl.value = t.variables.bookTitle || '';
            if (varChapterEl) varChapterEl.value = t.variables.chapter || '';
            if (varChapterSummaryEl) varChapterSummaryEl.value = t.variables.chapterSummary || '';
            if (varTargetLanguageEl) varTargetLanguageEl.value = t.variables.targetLanguage || '';
        ...
        save() {
            // ✅ FIX: Validar que hay template activo
            if (!State.activeTemplate) {
                UI.toast('❌ No hay plantilla activa para guardar', 'error');
                DebugLogger.log('❌ Intento de guardar sin template activo', 'error');
                return;
            }
            ...
            if (!templateNameEl || !systemPromptEl || !userPromptEl) {
                UI.toast('❌ Error: elementos de formulario no encontrados', 'error');
                DebugLogger.log('❌ Elementos DOM no encontrados al guardar', 'error');
                return;
            }
            ...
            const t = {
                name: templateNameEl.value || 'Sin nombre',
            ...
            DebugLogger.log(`✅ Plantilla "${t.name}" guardada`, 'success');
        },
